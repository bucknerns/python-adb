...
class AdbMessage(object):
    ...
    local_id/remote_id:
      Turns out the documentation is host/device ambidextrous, so local_id is
      the id for 'the sender' and remote_id is for 'the recipient'. So since
      we're only on the host, we'll re-document with host_id and device_id:
    ...
    def Unpack(cls, message):
        try:
            cmd, arg0, arg1, data_length, data_checksum, unused_magic = (
                struct.unpack(cls.format, message))
    ...
    def Read(cls, usb, expected_cmds, timeout_ms=None, total_timeout_ms=None):
        ...
        while True:
            ...
            if time.time() - start > total_timeout_ms:
                raise InvalidCommandError(
                    'Never got one of the expected responses (%s)' %
    ...
    def Connect(cls, usb, banner='notadb', rsa_keys=None, auth_timeout_ms=100):
        ...
        Args:
          ...
          rsa_keys: List of AuthSigner subclass instances to be used for
              authentication. The device can either accept one of these via the
              Sign method, or we will send the result of GetPublicKey from the
              first one if the device doesn't accept any of them.
          auth_timeout_ms: Timeout to wait for when sending a new public key.
              This is only relevant when we send a new public key. The device
              shows a dialog and this timeout is how long to wait for that
              dialog. If used in automation, this should be low to catch such
              a case as a failure quickly; while in interactive settings it
              should be high to allow users to accept the dialog. We default to
              automation here, so it's low by default.

        Returns:
          The device's reported banner. Always starts with the state (device,
              recovery, or sideload), sometimes includes information after a :
              with various product information.

        Raises:
          usb_exceptions.DeviceAuthError: When the device expects
            authentication, but we weren't given any valid keys.
        ...
        if cmd == 'AUTH':
            ...
            for rsa_key in rsa_keys:
                ...
                msg = cls(
                    command='AUTH', arg0=AUTH_SIGNATURE, arg1=0,
                    data=signed_token)
                msg.Send(usb)
    ...
    def Open(cls, usb, destination, timeout_ms=None):
        ...
        if local_id != their_local_id:
            raise InvalidResponseError(
                'Expected the local_id to be %s, got %s' %
        ...
        if cmd != 'OKAY':
            raise InvalidCommandError(
                'Expected a ready response, got %s' % cmd, cmd,
    ...
    def Command(cls, usb, service, command='', timeout_ms=None):
        ...
        response. All the data is held in memory, large responses will be slow
        and can fill up memory.
