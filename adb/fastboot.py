...
def DEFAULT_MESSAGE_CALLBACK(m):
    return logging.info('Got %s from device', m)
...
class FastbootProtocol(object):
    ...
    def HandleDataSending(self, source_file, source_len,
        ...
        Args:
          ...
          info_cb: Optional callback for text sent from the bootloader.
          progress_callback: Callback that takes the current and the total
            progress of the current file.
          timeout_ms: Timeout in milliseconds to wait for each response.

        Raises:
          FastbootTransferError: When fastboot can't handle this amount of
            data.
          FastbootStateMismatch: Fastboot responded with the wrong packet type.
          FastbootRemoteFailure: Fastboot reported failure.
          FastbootInvalidResponse: Fastboot responded with an unknown packet
            type.
        ...
        if accepted_size != source_len:
            raise FastbootTransferError(
                'Device refused to download %s bytes of data '
                '(accepts %s bytes)', source_len, accepted_size)
    ...
    def _Write(self, data, length, progress_callback=None):
        """Sends the data to the device, tracking progress with the
        callback."""
...
class FastbootCommands(object):
    ...
    def FlashFromFile(
        self, partition, source_file, source_len=0,
        ...
        Args:
          ...
          source_len: Optional length of source_file, uses os.stat if not
            provided.
    ...
    def Download(self, source_file, source_len=0,
        ...
        Args:
          source_file: A filename or file-like object to download to the
            device.
          source_len: Optional length of source_file. If source_file is a
              file-like object and source_len is not provided, source_file is
              read into memory.
          info_cb: Optional callback accepting FastbootMessage for text sent
            from the bootloader.
          progress_callback: Optional callback called with the percent of the
              source_file downloaded. Note, this doesn't include progress of
              the actual flashing.
        ...
        return self._protocol.HandleDataSending(
            source_file, source_len, info_cb,
            progress_callback=progress_callback)
    ...
    def Flash(self, partition, timeout_ms=0, info_cb=DEFAULT_MESSAGE_CALLBACK):
        ...
        Args:
          ...
          timeout_ms: Optional timeout in milliseconds to wait for it to
            finish.
    ...
    def Reboot(self, target_mode='', timeout_ms=None):
        ...
        Args:
            target_mode: Normal reboot when unspecified. Can specify other
                target modes such as 'recovery' or 'bootloader'.
            timeout_ms: Optional timeout in milliseconds to wait for a
                response.

        Returns:
            Usually the empty string. Depends on the bootloader and the
                target_mode.
        """
        return self._SimpleCommand(
