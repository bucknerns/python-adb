...
class UsbHandle(object):
    ...
    def __init__(self, device, setting, usb_info=None, timeout_ms=None):
        ...
        Arguments:
          ...
          setting: libusb setting with the correct endpoints to communicate
            with.
          usb_info: String describing the usb path/serial/device, for
            debugging.
    ...
    def BulkWrite(self, data, timeout_ms=None):
        if self._handle is None:
            raise usb_exceptions.WriteFailedError(
                'This handle has been closed, probably due to another being '
                'opened.', None)
        try:
    ...
    def BulkRead(self, length, timeout_ms=None):
        if self._handle is None:
            raise usb_exceptions.ReadFailedError(
                ...
                'opened.', None)
        try:
            # python-libusb1 > 1.6 exposes bytearray()s now instead of
            # bytes/str.
            # To support older and newer versions, we ensure everything's
            # bytearray() from here on out.
    ...
    @classmethod
    def Find(
        cls, setting_matcher, port_path=None, serial=None,
            timeout_ms=None):
